...
(function() {
    ...
    function saveSettings() {
        ...
        // Clear all overlays and flags to force a full re-render
    ...
    function getDisplaySettings(fullName) {
        ...
        if (clean.includes('Specialized Killstreak')) {
            ...
            if (fullName.includes('KB-808')) { color = '#FF9900'; initials = 'KB'; }
            else if (fullName.includes('Money Furnace')) { color = '#7B9095'; initials = 'MF'; }
            else if (fullName.includes('Taunt Processor')) { color = '#0099FF'; initials = 'TP'; }
            else if (fullName.includes('Emotion Detector')) { color = '#55FF55'; initials = 'ED'; }
            else if (fullName.includes('Bomb Stabilizer')) { color = '#FF4444'; initials = 'BS'; }
            else if (fullName.includes('Humor Suppression')) { color = '#E3E3C5'; initials = 'HS'; }
            else if (fullName.includes('Currency Digester')) { color = '#FF66AA'; initials = 'CD'; }
            else if (fullName.includes('Brainstorm Bulb')) { color = '#f5dc98'; initials = 'BB'; }
        }

        const robot = { 'Battle-Worn': "BR", 'Reinforced': "RI", 'Pristine': "PR" };
        for (let key in robot) {
            if (fullName.includes(key)) {
                clean = clean.replace(new RegExp(key + '(?: Robot)?', 'g'), '').trim();
                initials = `${robot[key]} ${initials || 'Part'}`;
                break;
            }
        }
    ...
    function parseItemDetails(descriptionArray) {
        ...
        for (let lineObj of descriptionArray) {
            ...
            if (text.includes('Sheen:')) {
                let sheenName = text.split('Sheen:')[1].trim();
            ...
            if (text.includes('Killstreaker:')) {
                let ksName = text.split('Killstreaker:')[1].trim();
                for (let key in KILLSTREAKER_DATA) {
                    if (ksName.includes(key)) { result.ksCode = KILLSTREAKER_DATA[key]; break; }
                }
    ...
    // =========================================================================
    // CORE RENDERING (Settings Aware)
    // =========================================================================
    function renderFabOverlay(item, details) {
    ...
    if (isTradePage) {
        const scanTrade = () => {
            ...
            if (settings.showSummary) {
                let html = '<div style="color: #fff; font-weight: bold; border-bottom: 1px solid #555; margin-bottom: 5px; font-size:10px;">MISSING PARTS</div>';
                let missingCount = 0;
                Object.keys(globalNeeded).sort().forEach(name => {
                    let net = globalNeeded[name] - (globalOwned[name] || 0);
                    if (net > 0) {
                        missingCount++;
                ...
                document.getElementById('fab_summary_list').innerHTML = (missingCount === 0) ? '<div style="color:#888; font-size:10px;">Complete!</div>' : html;
            }
        };
        ...
        // =========================================================================
        // MODE 2: INVENTORY PAGE (v3.3 Fixed Logic)
    // =========================================================================
    else {
        ...
        const updateInvSummary = () => {
            ...
            Object.keys(NEEDED_PARTS).sort().forEach(name => {
                ...
                if (missing > 0) {
                    ...
                    html += `<div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:2px;">
                                <span style="color:${s.color}; text-shadow:1px 1px 0 #000;">${s.text}</span>
                                <span style="color:#fff;">${missing}</span>
                             </div>`;
                }
            });
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    if (rawName.includes('Fabricator') || rawName.includes('Kit')) {
                        ...
                        details.ingredients.forEach(ing => {
                            NEEDED_PARTS[ing.name] = (NEEDED_PARTS[ing.name] || 0) + ing.count;
                        });
                    } else {
                        OWNED_PARTS[rawName] = (OWNED_PARTS[rawName] || 0) + 1;
                    }
                });
